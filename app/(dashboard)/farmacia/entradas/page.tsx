    ...
    <div className="max-w-5xl mx-auto space-y-4 w-full min-w-0">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
          ...
          <p className="text-xs text-slate-500">{entradas.length} registros</p>
        </div>

        <ul className="divide-y divide-slate-100">
          {entradas.map((e) => (
            <li key={e.id} className="px-4 py-3 flex flex-col md:flex-row md:items-center gap-2">
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-slate-900 truncate">
                  NF {e.numeroNota}
                  {e.serie ? <span className="text-xs font-normal text-slate-500"> • Série {e.serie}</span> : null}
                </p>
          ...
          {entradas.length === 0 ? (
            <li className="px-4 py-6 text-sm text-slate-500">Nenhuma entrada registrada.</li>
          ) : null}
        </ul>
      </div>
    </div>
